...
export interface AgyCapabilities {
  ...
  effortFlag: boolean;
  /** `--json-schema` exists → structured output schema enforcement is supported. */
  jsonSchemaFlag: boolean;
  /** `--mode` exists → execution mode (accept-edits|plan) is supported. */
  modeFlag: boolean;
  /** `--add-dir` exists → workspace directory injection is supported. */
  addDirFlag: boolean;
  ...
  /** `--sandbox` exists → terminal restriction sandbox is supported. */
  sandboxFlag: boolean;
