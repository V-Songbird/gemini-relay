...
function resolveAgy(command: string): string {
  ...
  if (process.platform === "win32") {
    // Probe common Windows install locations for Antigravity CLI
    const probeInstallDir = (): string | undefined => {
      const base = process.env.LOCALAPPDATA;
      const candidates = [
        base && path.join(base, "agy", "bin", `${command}.exe`),
        base && path.join(base, "Antigravity", `${command}.exe`),
        base && path.join(base, "Programs", "agy", `${command}.exe`),
...
export async function executeCommand(
  ...
  return new Promise((resolve, reject) => {
    ...
    const isExe = isWindows && resolvedCommand.toLowerCase().endsWith(".exe");
    const useShell = isWindows && !isExe;
    ...
    // `a&calc`); wrapping each arg in double quotes makes them inert. For direct
    // `.exe` binaries (like agy.exe or node.exe), shell:false avoids cmd.exe's
    // 8,191-char limit and multiline newline corruption.
    const safeArgs = useShell ? args.map(quoteForCmd) : args;
    const spawnCommand =
      useShell && /\s/.test(resolvedCommand) ? `"${resolvedCommand}"` : resolvedCommand;
