...
rm -rf .wiki-temp
git clone https://github.com/V-Songbird/gemini-relay.wiki.git .wiki-temp 2>/dev/null || {
    echo "⚠️  Wiki doesn't exist yet. Creating it through GitHub..."
    # Create initial wiki page through API
    gh api repos/V-Songbird/gemini-relay/wiki/pages \
        --method POST \
        -f title="Home" \
        -f body="Initializing wiki..." || true
    
    # Try cloning again
    git clone https://github.com/V-Songbird/gemini-relay.wiki.git .wiki-temp
}
...
## 📦 Quick Install
```bash
claude mcp add gemini-cli -- npx -y gemini-relay
```
EOF
...
**Quick Links**
* [📦 NPM Package](https://www.npmjs.com/package/gemini-relay)
* [🐙 GitHub Repo](https://github.com/V-Songbird/gemini-relay)
* [📋 Report Issue](https://github.com/V-Songbird/gemini-relay/issues/new)
EOF

# Create footer
cat > _Footer.md << 'EOF'
---
📄 [MIT License](https://github.com/V-Songbird/gemini-relay/blob/main/LICENSE) | 
🔧 [Contribute](https://github.com/V-Songbird/gemini-relay/blob/main/CONTRIBUTING.md) | 
📦 [NPM](https://www.npmjs.com/package/gemini-relay) |
⭐ [Star on GitHub](https://github.com/V-Songbird/gemini-relay)
EOF
...
echo "🔗 View at: https://github.com/V-Songbird/gemini-relay/wiki"
echo ""
