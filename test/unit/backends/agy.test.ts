...
describe("Backends: agy arg building", () => {
  test("forwards model flags and normalizes aliases", () => {
    assert.deepEqual(buildAgyArgs({ model: "gemini-3.8-flash-high" }), [
      "--model",
      "gemini-3.8-flash-high",
    ]);
    assert.deepEqual(buildAgyArgs({ model: "pro" }), [
      "--model",
      "gemini-3.1-pro-high",
    ]);
    assert.deepEqual(buildAgyArgs({ model: "flash" }), [
      "--model",
      "gemini-3.8-flash-high",
    ]);
  });
  ...
  test("forwards reasoning effort, execution mode, and json schema", () => {
    assert.deepEqual(buildAgyArgs({ effort: "high" }), ["--effort", "high"]);
    assert.deepEqual(buildAgyArgs({ mode: "plan" }), ["--mode", "plan"]);
    assert.deepEqual(buildAgyArgs({ jsonSchema: '{"type":"object"}' }), [
      "--json-schema",
      '{"type":"object"}',
    ]);
    assert.deepEqual(buildAgyArgs({ addDirs: ["src", "test"] }), [
      "--add-dir",
      "src",
      "--add-dir",
      "test",
