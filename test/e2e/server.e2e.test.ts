...
describe("MCP Protocol E2E: Server Lifecycle & System Tools", () => {
  test("lists every registered tool with a valid input schema", async (t) => {
    ...
    for (const expected of ["ask-gemini", "brainstorm", "fetch-chunk", "ping", "Help", "gemini-plan", "gemini-image", "gemini-models", "gemini-doctor"]) {
  ...
  test("gemini-models returns available models and backend information over MCP", async (t) => {
    const res = await callTool(t, server, { name: "gemini-models", arguments: {} });
    assert.equal(res.isError ?? false, false);
    const text = textOf(res);
    assert.match(text, /Active Backend:/);
    assert.match(text, /gemini-3\.8-flash/);
  });

  test("gemini-doctor returns environment diagnostic over MCP", async (t) => {
    const res = await callTool(t, server, { name: "gemini-doctor", arguments: {} });
    assert.equal(res.isError ?? false, false);
    const text = textOf(res);
    assert.match(text, /Gemini MCP Tool Doctor/);
